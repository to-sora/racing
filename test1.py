        ...
        for k in range(len(NtrainingI[i][j])):
            for z in range(len(NtrainingI[i][j][k])):
                if j==NoofHource-1:
                    ...
                    if z==2 or z==3:
                        NtrainingI[i][j][k][z]=float(trainingI[i][j][k][z+1])*10
                    elif z==1:
                        NtrainingI[i][j][k][z] = float(trainingI[i][j][k][z + 1]) / 100
                    elif z==4 :
                        NtrainingI[i][j][k][z] = float(trainingI[i][j][k][z + 1]) / 10
...
print()
NtestI=np.zeros((len(testI),NoofHource+1,NoofData,7),dtype=float)
        ...
        for k in range(len(NtestI[i][j])):
            for z in range(len(NtestI[i][j][k])):
                if j==NoofHource-1:
                    ...
                    if z==2 or z==3:
                        NtestI[i][j][k][z]=float(testI[i][j][k][z+1])*10
                    elif z==1:
                        NtestI[i][j][k][z] = float(testI[i][j][k][z + 1]) / 100
                    elif z==4 :
                        NtestI[i][j][k][z] = float(testI[i][j][k][z + 1]) / 10
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(7*2,activation="tanh"),
    tf.keras.layers.Dense(6*2,activation="relu"),
    tf.keras.layers.Dense(4*2,activation="tanh"),

    tf.keras.layers.Dense(1,activation="relu"),
    tf.keras.layers.Reshape((13, 6)),
